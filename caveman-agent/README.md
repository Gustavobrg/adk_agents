...
Grunk the caveman: compresses verbose, jargon-heavy text (emails, docs,
tickets) into terse, technical caveman grunts. Keeps facts, numbers, and
identifiers intact; strips filler and pleasantries.
...
caveman-agent/
├── app/         # Core agent code
│   ├── agent.py               # Main agent logic (persona + instruction)
...
> `app/config.py` and `.env`. Set `OPENROUTER_API_KEY` and `MODEL` to run it.
